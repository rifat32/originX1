...
<div class="card">
    ...
    <div class="card-body">
        ...
        <div class="alert alert-success" role="alert">{{Session::get('tag-updated')}}</div>
        @endif
      
        <form action="{{route('globalTag.update')}}" method="POST" >
    @csrf 
    <input type="hidden" value="{{$tag->id}}" name="id">
    <input type="hidden" value="{{$tag->service}}" name="service">
    <div class="form-group">
        <lebel for="tag">Tag Name (only one)</lebel>
        <input required value="{{$tag->tag}}" type="text" id="tag" name="tag" class="form-control">
    </div>
