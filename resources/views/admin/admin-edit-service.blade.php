...
<div class="card">
    ...
    <div class="card-body">
        ...
        <div class="alert alert-success" role="alert">{{Session::get('service-updated')}}</div>
        @endif
      
        <form action="{{route('service.update')}}" method="POST" >
    @csrf 
    <div class="form-group">
        <input type="hidden" name="id" value="{{$servicee->id}}">
        <lebel for="service">Service Name</lebel>
        <input value="{{$servicee->service}}" required type="text" id="service" name="service" class="form-control">
    </div>
